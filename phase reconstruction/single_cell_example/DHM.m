...
I=imread('hologram.jpg');%读所拍的细胞全息图
...
%xc=902;yc=788;        
xc=279;yc=244;                  % 要正确读取+1级频谱的中心坐标位置。修改数值
dx=18;%选取所需的频谱范围
dy=18;
hold on;                 
xt=[xc-dx xc-dx];
...
N=496;
M=496;
Ufi=zeros(N,M);% 预设一个窗口，准备将提取的成分移到图像中间
Ufibg=zeros(N,M);
...
Ufi(yb0-yc+M/2:yb1-yc+M/2,xb0-xc+N/2:xb1-xc+N/2)=FFTholography0(yb0:yb1,xb0:xb1);%像面滤波；提取Uf中左上角复振幅成分
Ufibg(yb0-yc+M/2:yb1-yc+M/2,xb0-xc+N/2:xb1-xc+N/2)=FFTbg0(yb0:yb1,xb0:xb1);%背景图一样的操作
Uf=Ufi;
Ufbg=Ufibg;
...
%% 最小二乘解包裹运算
[M,N]=size(phase);                 %计算二维包裹相位的大小（行、列数）
...
%% ROI选取
h1=drawfreehand;%创建平滑的ROI
mask=createMask(h1);
%figure,imshow(mask);
cell_unwrapped_phase=mask.*phase_unwrap_no_curve; %最大连通域作为细胞二值掩膜
figure,imshow(cell_unwrapped_phase,[]);colormap(jet);

%% 细胞数据保存
%cell_phase_single=cell_unwrapped_phase;
%cell_phase_single(find(cell_phase_single<0))=0; %减相位负值
%figure,imshow(cell_phase_single);colormap(parula);
%figure,mesh(cell_phase_single);colormap(parula)
%filename=['/Users/taylor/Desktop/image_cell_phase_',num2str(22)];   %保存细胞相位数据 %修改文件夹名称
%save(filename,'cell_phase_single');